...
const Dashboard = () => {
  ...
  // Derived: on-chain cxUSDC value in INR
  const cxUSDCRate = exchangeRates?.usdc || exchangeRates?.usdcToUsd || 1;
  const usdToInr = exchangeRates?.usdToInr || 83;
  const ethRate = exchangeRates?.ethToUsd || 2500;

  const cryptoNetWorth =
    (cryptoBalances?.cxUSDC || 0) * cxUSDCRate * usdToInr +
    (cryptoBalances?.eth || 0) * ethRate * usdToInr;

  const totalNetWorth = balance + cryptoNetWorth;
  const fiatPct = totalNetWorth > 0 ? ((balance / totalNetWorth) * 100).toFixed(1) : '0.0';
  const cryptoPct = totalNetWorth > 0 ? ((cryptoNetWorth / totalNetWorth) * 100).toFixed(1) : '0.0';
  ...
  const walletAddr = userWallet || convergeXWallet?.address;
  const shortAddr = walletAddr ? `${walletAddr.substring(0, 6)}...${walletAddr.slice(-4)}` : '...';
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="animate" exit="exit" className="space-y-8">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-4 flex justify-center gap-6 text-sm">
            <div className="flex items-center gap-1 text-blue-400">
              <Landmark size={14} />
              <span>Fiat: {fiatPct}%</span>
            </div>
            <div className="flex items-center gap-1 text-purple-400">
              <Wallet size={14} />
              <span>Crypto: {cryptoPct}%</span>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* RIGHT: CONVERGEX VAULT (ON-CHAIN) */}
        <motion.div variants={itemVariants} className="space-y-4">
          <div className="flex items-center gap-2 mb-2">
            <div className="p-2 bg-purple-500/20 rounded-lg text-purple-400"><Wallet size={20} /></div>
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-white font-medium text-sm">On-chain Wallet</p>
                    <p className="text-purple-300 text-xs flex items-center gap-1">
                      {shortAddr}
                      {walletAddr && (
                        <>
                          <button onClick={() => { navigator.clipboard.writeText(walletAddr); showToast.success("Address Copied") }} className="hover:text-white"><Copy size={10} /></button>
                          <EtherscanLink address={walletAddr} short />
                        </>
                      )}
                    </p>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-blue-500/10 rounded-xl p-3 border border-white/5">
                  <p className="text-xs font-bold text-blue-400 mb-1">cxUSDC</p>
                  <p className="text-white font-mono text-sm">{(cryptoBalances?.cxUSDC || 0).toFixed(4)}</p>
                </div>
                <div className="bg-gray-500/10 rounded-xl p-3 border border-white/5">
                  <p className="text-xs font-bold text-gray-300 mb-1">ETH (gas)</p>
                  <p className="text-white font-mono text-sm">{(cryptoBalances?.eth || 0).toFixed(6)}</p>
                </div>
              </div>
      ...
      {/* 3. QUICK ACTIONS */}
      <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-2 md:grid-cols-6 gap-4">
        <Link to="/send" className="glass-card p-4 flex flex-col items-center justify-center gap-3 hover:bg-white/10 transition-all group border-blue-500/20 hover:border-blue-500/50 hover:-translate-y-1">
          <div className="w-12 h-12 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400 group-hover:scale-110 transition-transform shadow-lg shadow-blue-500/10"><Send size={24} /></div>
        ...
        <Link to="/add-money" className="glass-card p-4 flex flex-col items-center justify-center gap-3 hover:bg-white/10 transition-all group border-green-500/20 hover:border-green-500/50 hover:-translate-y-1">
          <div className="w-12 h-12 rounded-full bg-green-500/20 flex items-center justify-center text-green-400 group-hover:scale-110 transition-transform shadow-lg shadow-green-500/10"><PlusCircle size={24} /></div>
        ...
        <Link to="/pay-crypto" className="glass-card p-4 flex flex-col items-center justify-center gap-3 hover:bg-white/10 transition-all group border-purple-500/20 hover:border-purple-500/50 hover:-translate-y-1">
          <div className="w-12 h-12 rounded-full bg-purple-500/20 flex items-center justify-center text-purple-400 group-hover:scale-110 transition-transform shadow-lg shadow-purple-500/10"><RefreshCw size={24} /></div>
        ...
        <Link to="/requests" className="glass-card p-4 flex flex-col items-center justify-center gap-3 hover:bg-white/10 transition-all group border-cyan-500/20 hover:border-cyan-500/50 hover:-translate-y-1">
          <div className="w-12 h-12 rounded-full bg-cyan-500/20 flex items-center justify-center text-cyan-400 group-hover:scale-110 transition-transform shadow-lg shadow-cyan-500/10"><ArrowLeftRight size={24} /></div>
        ...
        <Link to="/transactions" className="glass-card p-4 flex flex-col items-center justify-center gap-3 hover:bg-white/10 transition-all group border-emerald-500/20 hover:border-emerald-500/50 hover:-translate-y-1">
          <div className="w-12 h-12 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-400 group-hover:scale-110 transition-transform shadow-lg shadow-emerald-500/10"><TrendingUp size={24} /></div>
